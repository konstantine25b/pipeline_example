...
def forward_fill_weekends(input_csv: Path, output_csv: Path):
    """
    Read historical NBG data and create a new CSV with missing days forward-filled.
    - Sunday and Monday use Saturday's data with an added calendar_date field
    """
    print(f"Reading data from {input_csv}...")
    
    # Read all records from input CSV
    ...
    # Generate all dates in range and forward-fill
    all_records = []
    ...
    while current_date <= max_date:
        ...
        if iso_date in records_by_date:
            ...
            # Add calendar_date field to original records
            ...
            # Save Saturday data for Sunday/Monday forward-fill
            if weekday == 5:  # Saturday
                last_saturday_records = date_records
            
            print(f"  ✓ {current_date.date()} ({weekday_name}): {len(date_records)} records (original)")
        
        elif weekday in [6, 0]:  # Sunday (6) or Monday (0)
            # Forward-fill Sunday and Monday with Saturday's data
            if last_saturday_records:
                # Copy Saturday records and add calendar_date for the current day
    ...
    # Reorder fieldnames to put calendar_date and calendar_weekday first
    original_fields = list(all_records[0].keys())
    # Remove calendar fields if they exist
    original_fields = [f for f in original_fields if f not in ['calendar_date', 'calendar_weekday']]
    # Put calendar fields at the beginning
